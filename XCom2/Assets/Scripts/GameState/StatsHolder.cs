public class StatsHolder
{
	...
	// "); // this represent the timer it gets updated each frame lerpSpeed += Time.deltaTime;
	// float percentComplete = lerpSpeed / chipSpeed; percentComplete *= percentComplete; //
	// this is [0,1] value used to execute the mathf lerp method image.fillAmount =
	// Mathf.Lerp(currentValue, destination, percentComplete); yield return null; // if we the
	// percent reaches 1 => finish moving => reset the counter timer // and breal the loop
	...
	// int RestHealthUnit = Mathf.FloorToInt(SubjectRef.Health * maxHealthUnit /
	// SubjectRef.MaxHealth); //for (int i = frontHealthBar.transform.childCount - 1; i >
	// RestHealthUnit; i--) //{ //
